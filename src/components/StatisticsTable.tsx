...
import { CalendarDay, Member, calculateCostPerPerson, formatCurrency } from "@/utils/schedulerUtils";
    ...
    <div className="glass-card p-6 animate-slide-up">
                ...
                <tr key={member.id} className="border-b border-border/60 hover:bg-muted/20 transition-colors">
                  <td className="py-4">
                    <div className="flex items-center">
                      <div className="w-8 h-8 rounded-full bg-badminton flex items-center justify-center text-white text-sm mr-3">
      ...
      <div className="mt-6 p-4 bg-badminton-light rounded-lg border border-badminton/20">
        <p className="text-sm text-center text-badminton-dark">
          Chi phí mỗi buổi: <span className="font-bold">{formatCurrency(260000)}</span> chia đều cho số người tham gia
        </p>
      </div>
    </div>
  );
