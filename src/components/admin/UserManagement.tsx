...
const UserManagement: React.FC<UserManagementProps> = ({
  ...
  const handleBlockUser = async (userId: string, isBlocked: boolean) => {
    ...
    try {
      ...
      toast.error(
        `Có lỗi xảy ra khi ${isBlocked ? "mở khóa" : "khóa"} người dùng`
      );
    } finally {
                ...
                <div
                  ...
                  className={`p-3 rounded-lg border ${
                    selectedUser?.id === user.id
                      ? "border-badminton bg-badminton/5"
                      : "border-border"
                  } hover:border-badminton cursor-pointer transition-all`}
                  onClick={() => {
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Tự động tham gia mọi buổi tập
                      </p>
                    </div>
                    <Switch
                      checked={selectedUser.is_core}
                      onCheckedChange={() =>
                        handleToggleCoreMember(
                          selectedUser.id,
                          selectedUser.is_core
                        )
                      }
                    />
                  </div>
